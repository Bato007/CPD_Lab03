...
int main(void) {
  ...
  Generate_Vector_Random(local_x, local_n, n, "x", my_rank, comm);
  Generate_Vector_Random(local_y, local_n, n, "y", my_rank, comm);

  Print_vector(local_x, local_n, n, "x", my_rank, comm);
  Print_vector(local_y, local_n, n, "y", my_rank, comm);

  Read_e(&local_e, my_rank, comm);

  tstart = MPI_Wtime();
  Product_Scale(local_x, &local_e, local_n);
  Product_Scale(local_y, &local_e, local_n);
  tend = MPI_Wtime();
...
void Read_e(
  ...
  MPI_Bcast(local_e, 1, MPI_INT, 0, comm);
}  /* Read_e */
...
/*-------------------------------------------------------------------
 * Function:   Generate_Vector_Random
 * Purpose:    Generate random vector on process 0 and distribute
...
void Generate_Vector_Random(
  ...
  int i;
  int local_ok = 1;
  char* fname = "Generate_Vector_Random";
